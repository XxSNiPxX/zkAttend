...
describe("Geofencing", () => {
  ...
  it("should set geofence so that a user can send a request", async () => {
    const aliceString = alice.toBase58();
    ...
    const tmp_event = "Myevent";
    const tmp_description = "some description";

    const response = await fetch(
      `http://localhost:3000/api/get/geofence/createGeofence?publicKey=${aliceString}&lat=37.7749&long=-122.4194&radius=100&events=${encodeURIComponent(
        tmp_event
      )}&description=${encodeURIComponent(tmp_description)}`
    );
    ...
    const circuitEvent = CircuitString.fromString(tmp_event);
    const circuitDescription = CircuitString.fromString(tmp_description);
  ...
  it(
    "should allow RSVP if a valid proof is provided",
    async () => {
      ...
      const nullstring = nullifier.getPublicKey().toBase58();
      const geostring = alice.toBase58();
      ...
      const rsvpdata =
        (await rsvpresponse.json()) as JsonCompatibleRSVPSignature;

      console.log(rsvpdata);

      const nullField = nullifier.getPublicKey().toFields();
      const geoField = alice.toFields();
      ...
      rsvpProof = await realProof(
        await canRSVP(witness, nullifier, geoFenceSignature, alice)
      );
